...
seas.2 = read.csv("data/env03.csv")
names(env.seaso)
names(env.daily)

veg = read.csv("~/Downloads/veg01.csv")
...
rownames(seas.2) = paste(seas.2$node, seas.2$year, sep = "-")

#okay so 21 also excluded here in the gps list, gonna ads it back in 
...
seas.2 = seas.2[row.names(seas.2) %in% gps, ]
...
seas.2 = seas.2[, c("MEAN_SM_5", "MEAN_ST_5", "VAR_ST_5_tc", "VAR_SM_5_tc", "PRED_SM_5", "PRED_ST_5")]
...
rda_pretty_plot = function(rda_obj, displayText = FALSE){
  ...
  sc_bp <- scores(rda_obj, display="bp", choices=c(1, 2), scaling=2)
  sc_sites <- scores(rda_obj, display="sites", choices=c(1, 2), scaling=2)
  sc_species = scores(rda_obj, display = "species", choices = c(1,2), scaling = 3)
...
#Added this line that creates the output directory in the current working directory. If it already exists
#this just throws a warning and doesn't do anything.
dir.create('output')

png("output/fig2a.png", height = 4, width = 4, res = 300, units = "in")
par(mar = c(4,4,1,1), family = "Times New Roman")
rda_pretty_plot(r_mod1)
dev.off()

png("output/fig2b.png", height = 4, width = 4, res = 300, units = "in")
par(mar = c(4,4,1,1), family = "Times New Roman")
rda_pretty_plot(r_mod2)
dev.off()

png("output/fig2c.png", height = 4, width = 4, res = 300, units = "in")
par(mar = c(4,4,1,1), family = "Times New Roman")
...
seas.2 = seas.2[,c(1,2,4,3,5,6)]

colnames(env.daily) = c("MSM", "MST", "VSM", "VST", "PSM", "PST")
colnames(env.seaso) = c("MSM", "MST", "VSM", "VST", "PSM", "PST")
colnames(seas.2) = c("MSM", "MST", "VSM", "VST", "PSM", "PST")


png("output/Figure_S1.png", height = 8, width = 8, res = 200, units = "in")
chart.Correlation(env.daily, histogram = TRUE, method = "pearson")
dev.off()

png("output/Figure_S2.png", height = 8, width = 8, res = 200, units = "in")
chart.Correlation(seas.2, histogram = TRUE, method = "pearson")
dev.off()
